...
private const uint MOD_MASK_2BPP = 0b111u;
private const uint MOD_MASK_4BPP = 0b11u;

// Modulation Values

private static readonly byte[] MOD_VALUES_2BPP = [ 0, 3, 5, 8 ];
private static readonly byte[] MOD_VALUES_4BPP = [ 0, 2, 4, 5, 6, 8 ];

// Modulation bit pos (2bbp)

private static readonly byte[][] MOD_POS_2BPP =
[
...
private static readonly byte[][] MOD_POS_4BPP =
[
...
// Extract Modulation flags

private static uint ExtractMod(uint data, int x, int y, bool is2BPP)
{
int bitPos = is2BPP ? MOD_POS_2BPP[y][x] : MOD_POS_4BPP[y][x];
uint mask = is2BPP ? MOD_MASK_2BPP : MOD_MASK_4BPP;

uint mod = (data >> bitPos) & mask;
var max = is2BPP ? (uint)MOD_VALUES_2BPP.Length : (uint)MOD_VALUES_4BPP.Length;

return Math.Min(mod, max - 1);
}

// Insert Modulation flags

private static uint InsertMod(uint data, uint mod, int x, int y, bool is2BPP)
{
int bitPos = is2BPP ? MOD_POS_2BPP[y][x] : MOD_POS_4BPP[y][x];

return data | (mod << bitPos);
}

// Apply Modulation

private static TextureColor16 ApplyModulation(in TextureColor16 colorA, in TextureColor16 colorB,
                                              uint mod, bool is2BPP)
{
const int WEIGHT_SCALE = 8;

int wB = is2BPP ? MOD_VALUES_2BPP[mod] : MOD_VALUES_4BPP[mod];
int wA = WEIGHT_SCALE - wB;

return TextureHelper.LerpColors(colorA, colorB, wA, wB, false);
}

// Evaluate Color

private static TextureColor16 EvaluateColor(in TextureColor16 colorA, in TextureColor16 colorB, uint mod,
                                            bool hardTransition, bool is2BPP)
{

if(hardTransition)
return mod < 2 ? colorA : colorB;

return ApplyModulation(colorA, colorB, mod, is2BPP);
}

// Get Color diff

private static TextureColor16 GetColorDiff(in TextureColor16 colorA, in TextureColor16 colorB, uint mod,
                                           bool modInterpolate, bool hardTransition, bool is2BPP)
{

if(modInterpolate)
return ApplyModulation(colorA, colorB, mod, is2BPP);

return EvaluateColor(colorA, colorB, mod, hardTransition, is2BPP);
}
...
var maxMod = is2BPP ? (uint)MOD_VALUES_2BPP.Length : (uint)MOD_VALUES_4BPP.Length;

for(uint mod = 0; mod < maxMod; mod++)
{
var c = EvaluateColor(colorA, colorB, mod, hardTransition, is2BPP);
...
data = InsertMod(data, best, x, y, is2BPP);
}
...
uint mod = ExtractMod(modData, x, y, is2BPP);
var diff = GetColorDiff(colorA, colorB, mod, modInterpolate, hardTransition, is2BPP);
...
{
uint mod = ExtractMod(modData, x, y, is2BPP);
var final = GetColorDiff(colorA, colorB, mod, modInterpolate, hardTransition, is2BPP);
